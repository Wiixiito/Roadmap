$("body").on('click', "button#guardar", function() {
   // ev.preventDefault(); // evita que se envie el formulario
    ...
    var descripcion = $('.bodyTotal').find('input#descripcion').val();
    ...
    swal({
        ...
        if (confirma) {
            GuardarDiccionario(nombre,descripcion);
        } else {
            //limpiar.click();
        }
    });
...
function GuardarDiccionario(nombre,descripcion) {
    
    $.ajax({
        async:false,
        method: "POST",
        url: "Ajax/Aj_Diccionario.php",
        data: {
            Requerimiento: "GuardarDiccionario",
            Nombre: nombre,
        ...
        if (respuesta[0] == true) {
            swal("Tablero Kanban", "Sprint Guardado!", "success").then((confirma) => {
                try{

             $('.bodyTotal').find('input#nombre').val("");
             $('.bodyTotal').find('input#descripcion').val("");
                }catch(error){}
